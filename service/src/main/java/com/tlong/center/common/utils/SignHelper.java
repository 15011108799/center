...
import com.tlong.center.app.ESignBackController;
...
public class SignHelper {
	 ...
	 */
	public static Result initProject() {

		ProjectConfig proCfg = new ProjectConfig();

		// 项目ID(应用ID)
		proCfg.setProjectId(ESignBackController.projectId);
		// 项目Secret(应用Secret)
		proCfg.setProjectSecret(ESignBackController.projectSecret);
		// 开放平台地址
		proCfg.setItsmApiUrl(ESignBackController.url_SDKinit);
	 ...
	 * SignType.Single);
	 */
	public static FileDigestSignResult platformSignByStreamm(String srcPdfFile,String page,int x,int y,int width) {
		// 设置文件流签署的PDF文档信息
		...
		PosBean posBean = setXYPosBean(page,x,y,width);
	...
	/***
	 * 平台自身PDF摘要签署（文件二进制流）； 盖章位置通过关键字定位； 使用到接口：SelfSignServiceFactory.instance();
	 * selfSignService.localSignPdf(signPDFStreamBean, posBean, sealId,
	 * SignType.Single);
	 */
	public static FileDigestSignResult platformSignByStreammByKey(String srcPdfFile,String key,int x,int y,int width) {
		// 设置文件流签署的PDF文档信息
		SignPDFStreamBean signPDFStreamBean = setSignPDFStreamBean(FileHelper.getBytes(srcPdfFile));
		// 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效；
	 ...
	 * SignType.Single);
	 */
	public static FileDigestSignResult platformSignByFile(String srcPdfFile,String signedPdfFile,String page,int x,int y,int width) {
		// 设置文件签署的PDF文档信息
	 ...
	 */
	public static AddAccountResult addPersonAccount(String name,String idNo,int personArea) {
...
//		personBean.setMobile("152****4800");
	 ...
	 */
	public static AddAccountResult addOrganizeAccount(String name,int organType,int regType,String organCode,int userType,String agentName,String agentIdNo,String legalName,int legalArea,String legalIdNo) {
...
//		organizeBean.setAddress("杭州城落霞峰7号");
		...
		case 2:
			...
			// 法定代表人归属地,0-大陆，1-香港，2-澳门，3-台湾，4-外籍，默认0
			 organizeBean.setLegalArea(legalArea);
