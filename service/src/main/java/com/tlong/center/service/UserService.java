...
//                    .and(tlongUser.id.ne(requestDto.getUserId()))
    ...
    private Page<TlongUser> childrenUserIds(TlongUser user,PageRequest pageRequest,Predicate[] predicates){
        Iterable<WebOrg> all;
        if(user.getUserType() == null){
            ...
            if (Objects.nonNull(all)){
                ...
                //然后查询出这些机构下所有的代理商
                return appUserRepository.findAll(tlongUser.orgId.in(orgIds)
                                .and(predicates[0]),pageRequest);
            }else {
                //当前机构下没任何机构 返回空
                return null;
            }
    ...
    public Page<TlongUserResponseDto> findAllSuppliers(PageAndSortRequestDto requestDto, Long userId, MultiValueMap<String,String> params) {
        ...
        //获取所有的供应商当前用户的机构下面的
        Page<TlongUser> users;
        //获取机构集合
        Iterable<WebOrg> all = webOrgRepository.findAll(QWebOrg.webOrg.orgClass.eq(4)
                .and(QWebOrg.webOrg.parentOrgId.eq(user.getOrgId())));
        List<WebOrg> webOrgs = ToListUtil.IterableToList(all);
        if (CollectionUtils.isNotEmpty(webOrgs)){
            List<Long> collect = webOrgs.stream().map(WebOrg::getId).collect(Collectors.toList());
//            collect.add(user.getOrgId());
            users = appUserRepository.findAll(tlongUser.userType.eq(0)
                    .and(tlongUser.orgId.in(collect))
    ...
    public SuppliersRegisterRequsetDto findOne(Long id) {
        ...
        if (Objects.nonNull(one)) {
            registerRequsetDto.setOrgId(one.getId());
...
//            tlongUser.setOrgId(one1.getId());
     ...
     * 查询所有代理商
     */
    public Page<TlongUserResponseDto> findAllAgents(PageAndSortRequestDto pageAndSortRequestDto, Long userId, MultiValueMap<String,String> params) {
        TlongUser user = appUserRepository.findOne(userId);
        if (Objects.isNull(user)){
            throw new CustomException("当前用户不存在!");
        }
        ...
        Predicate[] predicates = resove(params);

        Page<TlongUser> users = this.childrenUserIds(user, pageRequest,predicates);
        Map<Long, String> orgNameMap = this.orgNameMap(users);
